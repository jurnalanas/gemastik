...
$(document).ready(function() {
	...
	function fnFormatDetails ( nTr )
		{
		 ...
		 if(aData != null){
			...
			var sOut = '<table width="100%" height="100" border="0" cellpadding="0" cellspacing="0">';
  				sOut += '<tr>';
    			sOut += '<td width="10%" rowspan="3"><div align="center"><img src="<?php echo base_url(); ?>upload/logo/'+aData[5]+'" width="100" height="100"></div></td>';
				sOut += '<td width="10%"><strong>Latitude</strong></td>';
				sOut += '<td width="2%"><strong>:</strong></td>';
				sOut += '<td width="56%">'+aData[6]+'</td>';
				sOut += '</tr>';
				sOut += '<tr>';
	...
	$('#tabels tbody').on('click','tr', function () {
	 ...
	 if(aData != null){
		...
		$('#markers_id').val(aData[9]);
		$('#remove_location_list_id').val(aData[9]);
		...
		$('#markers_address').val(aData[4]);
	 	$('#markers_lat').val(aData[6]);
		$('#markers_lng').val(aData[7]);
		$('#markers_desc').val(aData[8]);
